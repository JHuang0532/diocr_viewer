...
## tasks
請先依序完成下面任務
[X]1. 畫面左上角有一個載入功能,這個就是我們要輸入根目錄的地方,輸入之後點擊確認,系統開始輪巡該目錄,並找到上面的圖片資料夾,也就是上面提到的tbb08_300..
...
[X]3. 使用者點擊該筆圖片,例如他點到tbb08_300,那就讀取我上面提到的目錄內容,中間顯示bbox_mask_preview.jpg(表單被框取的位置圖片),右側顯示每一個欄位OCR的結果,

當使用者點及左側選單內的否一圖片,右側要顯示相關的預覽圖片及OCR辨識的結果,請參考我上面提供的目錄價購說明,將需要的資料在爬曲目路的同時整理出來並透過icp方式傳送給前端
