...
2. 將資料夾顯示在左側的清單上(就像vscode那樣可以看見目錄,但不用展開,只是用來給使用者選擇要看哪一筆資料)
3. 使用者點擊該筆圖片,例如他點到tbb08_300,那就讀取我上面提到的目錄內容,左側顯示bbox_mask_preview.jpg(表單被框取的位置圖片),右側顯示每一個欄位OCR的結果,
